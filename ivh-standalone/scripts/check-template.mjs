...
/* 本技能只做「独立成片」：PURPOSE 恒为 standalone，画面自己扛叙事、居中构图。
   ★ 透明素材（一块 3:4 的板）不归本技能管 —— 那是 ivh-overlay 的产物。 */
if (PURPOSE === "overlay") {
  console.log("  ! PURPOSE=overlay —— 本技能只做独立成片（standalone）。");
  console.log("    透明素材请改用 ivh-overlay 技能；整屏叠加素材不在本技能职责内。");
}
const KIND_LABEL = { text: "分幕（BASE=text）", timeline: "时间轴（BASE=timeline）" };
...
if (!cfgBlock) { fail("找不到 CONFIG 对象"); }
  ...
  const AUTOPLAY = pickBool("AUTOPLAY");

  if (got.RATIO && ["16:9", "4:3"].includes(got.RATIO)) note(`横版系布局（RATIO=${got.RATIO}）→ 用横向布局模板`);
  if (got.RATIO && ["9:16", "3:4"].includes(got.RATIO))  note(`竖版系布局（RATIO=${got.RATIO}）→ 用纵向布局模板`);
  if (got.PURPOSE === "overlay") note("PURPOSE='overlay' —— 本技能只做独立成片；透明素材请改用 ivh-overlay");
  if (got.BG === "transparent" && HIDE_HUD !== "true") note("BG='transparent' 但 HIDE_HUD 不为 true —— 导出前记得改");
  if (got.BASE === "text" && AUTOPLAY !== "true") note("BASE='text' 且未开启自动轮播 —— 录屏时需手动换幕");
  if (ANIMATE === "false") note("ANIMATE=false —— 阶段二 A 静态布局确认模式，无任何动画");
...
/* ==========================================================================
   9 · 时间轴完整性（BASE=timeline 时启用）
   --------------------------------------------------------------------------
   重叠判据按形态分流：
     素材模式   任意两个素材点的时间区间不得重叠（整块板同一时刻只放一组元素）
                且相邻素材点之间要留 >=1.5s 纯透明空档
     整屏叠加   同 data-safe 泊位不得重叠（不同泊位可以共存）
     独立成片   允许同场共存（幕内错峰），只给提示
   ========================================================================== */
head("9. 时间轴完整性");
...
if (BASE !== "timeline") {
  ...
  push(`  形态 ${KIND_LABEL[BASE] || BASE} · 检测到组件 ${compTags.length} 个`);
  ...
  if (compTags.length === 0) {
    ...
    if (missIn.length === 0 && missOut.length === 0) {
      ...
      /* —— 同场共存 ——
         独立成片里画面自己扛叙事，多个组件错峰同场是刻意的（主数据 + 归因标签），
         不算违规。只统计并提示落点，交给第 10 项的组件多样性去管节奏。 */
      const sorted = [...comps].sort((a, b) => a.in - b.in);
      let co = 0;
      for (let i = 0; i < sorted.length; i++)
        for (let j = i + 1; j < sorted.length; j++)
          if (sorted[i].in < sorted[j].out && sorted[j].in < sorted[i].out) co++;
      ok(`同场共存 ${co} 组（独立成片的幕内错峰，正常）`);
      if (co > 0) {
        const noRoll = comps.filter(c => !/data-roll=/.test(c.raw)).length;
        note(noRoll
          ? `${noRoll} 个组件没写 data-roll —— 同场共存时两条会叠在画面正中。写 data-roll="A"（主张，上半带）/ "B"（证据，下半带）自动分带`
          : "同场组件已按 data-roll 分带（A 上半带 / B 下半带），不会撞位");
      }
      ...
      /* 序列化展开提示：同组组件若全部同 in 点则偏生硬 */
      const inGroups = [...new Set(comps.map(c => c.in))];
      if (inGroups.length === 1 && comps.length >= 3) note("所有组件入点相同 —— 建议错峰入场（序列化展开）");
    }
  }
}
...
/* ==========================================================================
   10 · 叙事结构（独立成片专属）
   --------------------------------------------------------------------------
   BASE='text'     查幕：幕数、每幕停留、组件是否换着用（不要一个卡片用到死）
   BASE='timeline' 幕由组件承担，只查组件多样性
   ========================================================================== */
head("10. 叙事结构");
{
  /* —— 幕结构（BASE=text）—— */
  if (BASE !== "text") {
    note(`BASE='${BASE}'，幕由组件承担，幕结构不适用`);
  } else {
    const scenes = [...codeNoCss.matchAll(/<section[^>]*class="[^"]*\bscene\b[^"]*"[^>]*>/g)];
    scenes.length === 0
      ? fail("BASE='text' 但一个 .scene 都没有")
      : ok(`检测到 ${scenes.length} 幕`);

    if (scenes.length === 1) note("只有 1 幕 —— 独立成片通常需要 3 幕以上才有叙事弧（开场 / 主体 / 收尾）");

    /* 每幕停留 data-sec */
    const secs = scenes.map(m => {
      const v = (m[0].match(/data-sec="([\d.]+)"/) || [])[1];
      return v === undefined ? null : parseFloat(v);
    });
    const missSec = secs.filter(s => s === null || Number.isNaN(s));
    if (missSec.length) {
      fail(`${missSec.length} 幕缺少 data-sec，无法算时长`);
    } else {
      const bad = secs.filter(s => s < 3 || s > 12);
      bad.length === 0
        ? ok(`每幕停留 3~12s（共 ${secs.reduce((a, b) => a + b, 0).toFixed(1)}s）`)
        : note(`${bad.length} 幕停留不在 3~12s（${bad.join(" / ")}s）—— 太短赶、太长拖`);
    }
  }

  /* —— 组件多样性 ——
     A-roll 用 h1 / h2 / kicker（标题排版），B-roll 用 c-*（数据组件），都要算。 */
  const used = [...new Set(
    [...codeNoCss.matchAll(/class="[^"]*\b(c-[a-z]+|h1|h2|h3|kicker)\b/g)].map(m => m[1])
  )];
  if (used.length === 0) {
    skip("没用到任何组件（可能还只是纯文本幕）");
  } else {
    used.length >= 3
      ? ok(`用到 ${used.length} 种组件：${used.join(" / ")}`)
      : note(`只用到 ${used.length} 种组件：${used.join(" / ")} —— 一种卡片用到死会很单调，建议 >=3`);
  }
}
